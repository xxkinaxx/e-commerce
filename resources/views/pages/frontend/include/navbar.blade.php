                ...
                <div class="w-auto">
                    <ul class="fixed bg-white inset-0 flex flex-col invisible items-center justify-center opacity-0 md:visible md:flex-row md:bg-transparent md:relative md:opacity-100 md:flex md:items-center" id="menu">
                        ...
                        <li class="mx-3 py-6 md:py-0">
                            <a href="{{ route('detail.category', $row->slug) }}" class="text-black md:text-white hover:underline">{{ $row->name }}</a>
                        </li>
                        @endforeach
                        @if (Auth::check())
                        @if (Auth::user()->role === 'admin')
                        <li class="mx-3 py-6 md:py-0">
                            <a href="{{ route('admin.dashboard') }}" class="text-black md:text-white hover:underline">
                                Dashboard
                            </a>
                        </li>
                        @else
                        <li class="mx-3 py-6 md:py-0">
                            <a href="{{ route('user.dashboard') }}" class="text-black md:text-white hover:underline">
                                Dashboard
                            </a>
                        </li>
                        @endif
