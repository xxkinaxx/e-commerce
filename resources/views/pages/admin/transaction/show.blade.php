...
<div class="card">
    <div class="card-body">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                @foreach ($transaction->transactionItem as $detail)
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $detail->product->name }}</td>
                    <td>Rp. {{ number_format($detail->product->price) }}</td>
                    <td>
                        <img src="{{ $detail->product->productGallery()->exists() ? url('storage/product/gallery', $detail->product->productGallery->first()->image) : 'https://via.placeholder.com/100' }}" width="250">
                    </td>
